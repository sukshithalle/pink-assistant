...
class YouTubeController:
    ...
    def next_video(self):
        ...
        try:
            ...
            time.sleep(0.1)
            # Attempt Shift+N by sending 'n' (player interprets 'n' too)
            pyautogui.press('n')
            return True
        except Exception:
            try:
...
# ----------------- YouTube commands (added) -----------------
        ...
        if m:
            ...
            self.voice.speak(f"Opening YouTube and searching for {query}")
    ...
    def run(self):
        ...
        while True:
            ...
            if not text:
                time.sleep(0.7)
                continue
            if any(w in text for w in ["pink", "ping", "pinku","assiatant","buddy","green"]):
